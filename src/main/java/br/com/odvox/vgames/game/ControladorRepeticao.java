...
@ManagedBean
@SessionScoped
public class ControladorRepeticao implements Serializable{
	...
	private final String INDEX = "/sce.jsf?faces-redirect=true";
	...
	private void configurarBean() {
		...
		if (null != this.polly) {
			...
			if (null != this.voice) { 
				...
				if (req.getRequestURL().toString().contains("sce")) {
					this.carregarSaudacaoComPrimeiraPergunta();
				} else {
